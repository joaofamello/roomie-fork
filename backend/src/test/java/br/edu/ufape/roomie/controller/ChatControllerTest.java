...
class ChatControllerTest {
  ...
  private MessageResponseDTO sampleMessage() {
    return new MessageResponseDTO(
        200L, 100L, 1L, "Proprietário", "Olá!", LocalDateTime.of(2026, 3, 5, 13, 0), false);
  }
  ...
  void deveRetornar403QuandoNaoDono() throws Exception {
    ...
    when(chatService.startChat(any(), any()))
        .thenThrow(
            new IllegalStateException("Apenas o proprietário do imóvel pode iniciar um chat."));
    ...
    assertThat(response.getContentAsString())
        .isEqualTo("Apenas o proprietário do imóvel pode iniciar um chat.");
  }
  ...
  void deveRetornar200ComListaDeChats() throws Exception {
    ...
    var response = mvc.perform(get("/api/chats")).andReturn().getResponse();
  ...
  void deveRetornar200ComDetalhesDoChat() throws Exception {
    ...
    var response = mvc.perform(get("/api/chats/100")).andReturn().getResponse();
  ...
  void deveRetornar404QuandoChatNaoEncontrado() throws Exception {
    ...
    var response = mvc.perform(get("/api/chats/999")).andReturn().getResponse();
  ...
  void deveRetornar200ComMensagens() throws Exception {
    ...
    var response = mvc.perform(get("/api/chats/100/messages")).andReturn().getResponse();
  ...
  void deveRetornar404AoBuscarMensagensChatInexistente() throws Exception {
    ...
    var response = mvc.perform(get("/api/chats/999/messages")).andReturn().getResponse();
  ...
  void deveRetornar400ComMensagemVazia() throws Exception {
    ...
    assertThat(response.getContentAsString())
        .isEqualTo("O conteúdo da mensagem não pode ser vazio.");
  }
