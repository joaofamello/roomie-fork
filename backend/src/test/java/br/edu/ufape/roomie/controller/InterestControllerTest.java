...
class InterestControllerTest {
  ...
  @Autowired private JacksonTester<List<InterestSummaryDTO>> dtoListJacksonTester;
    ...
    SecurityContextHolder.getContext()
        .setAuthentication(
            new UsernamePasswordAuthenticationToken(
                mockStudent, null, mockStudent.getAuthorities()));

    when(studentRepository.findById(1L)).thenReturn(Optional.of(mockStudent));

    var response = mvc.perform(post("/announcements/1/interest")).andReturn().getResponse();

    response.setCharacterEncoding(StandardCharsets.UTF_8.name());
    assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
    assertThat(response.getContentAsString())
        .isEqualTo("Interesse registrado com sucesso. O administrador foi notificado.");
  }

  @Test
  @DisplayName(
      "Deveria devolver código HTTP 409 quando o usuário já demonstrou interesse anteriormente")
  void testaRegistrarInteresseDuplicado() throws Exception {
    ...
    assertThat(response.getContentAsString())
        .isEqualTo("Você já demonstrou interesse neste imóvel.");
  }
  ...
  void testaRegistrarInteresseImovelNaoEncontrado() throws Exception {
    ...
    doThrow(new RuntimeException("Imóvel não encontrado."))
        .when(interestService)
        .registerInterest(eq(999L), any());

    var response = mvc.perform(post("/announcements/999/interest")).andReturn().getResponse();
  ...
  void testaRegistrarInteresseUsuarioNaoEstudante() throws Exception {
    ...
    assertThat(response.getContentAsString())
        .isEqualTo("Acesso negado: Apenas estudantes podem demonstrar interesse em imóveis.");
  }

  @Test
  @DisplayName(
      "Deveria devolver HTTP 200 e a lista de interessados (Ficha Resumida) para o proprietário")
  void testaListarInteressadosComSucesso() throws Exception {
    ...
    var response = mvc.perform(get("/announcements/1/interests")).andReturn().getResponse();
  ...
  void testaListarInteressadosAcessoNegado() throws Exception {
    ...
    SecurityContextHolder.getContext()
        .setAuthentication(
            new UsernamePasswordAuthenticationToken(
                mockNotOwner, null, mockNotOwner.getAuthorities()));
    ...
    assertThat(response.getStatus()).isEqualTo(HttpStatus.FORBIDDEN.value());
    assertThat(response.getContentAsString())
        .isEqualTo("Acesso negado: Apenas o proprietário pode visualizar os interessados.");
  }
  ...
  void testaAtualizarStatusComSucesso() throws Exception {
    ...
    var response =
        mvc.perform(patch("/announcements/interests/1/status").param("status", "ACCEPTED"))
            .andReturn()
            .getResponse();
    ...
    assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
    assertThat(response.getContentAsString())
        .isEqualTo("Status da proposta atualizado para ACCEPTED");
  }

  @Test
  @DisplayName(
      "Deveria devolver HTTP 403 ao tentar atualizar status sem permissão ou em proposta já processada")
  void testaAtualizarStatusAcessoNegado() throws Exception {
    ...
    doThrow(new IllegalStateException("Não é possível alterar uma proposta que já foi processada."))
        .when(interestService)
        .updateInterestStatus(eq(1L), eq(InterestStatus.REJECTED), any(User.class));

    var response =
        mvc.perform(patch("/announcements/interests/1/status").param("status", "REJECTED"))
            .andReturn()
            .getResponse();
    ...
    assertThat(response.getStatus()).isEqualTo(HttpStatus.FORBIDDEN.value());
    assertThat(response.getContentAsString())
        .isEqualTo("Não é possível alterar uma proposta que já foi processada.");
  }
  ...
  void testaAtualizarStatusNaoEncontrado() throws Exception {
    ...
    doThrow(new RuntimeException("Interesse não encontrado."))
        .when(interestService)
        .updateInterestStatus(eq(999L), eq(InterestStatus.ACCEPTED), any(User.class));

    var response =
        mvc.perform(patch("/announcements/interests/999/status").param("status", "ACCEPTED"))
            .andReturn()
            .getResponse();
  ...
  void testaCheckInterestEstudanteComInteresse() throws Exception {
    ...
    var response = mvc.perform(get("/announcements/1/interest/check")).andReturn().getResponse();
  ...
  void testaListarInteressadosImovelNaoEncontrado() throws Exception {
    ...
    var response = mvc.perform(get("/announcements/999/interests")).andReturn().getResponse();
