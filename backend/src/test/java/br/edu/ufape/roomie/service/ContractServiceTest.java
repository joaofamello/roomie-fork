...
class ContractServiceTest {
  ...
  void acceptContract_ShouldThrowException_WhenUserIsNotTheStudent() {
    ...
    assertEquals(
        "Apenas o estudante destinatário pode aceitar o contrato.", exception.getMessage());
  }
  ...
  void rejectContract_ShouldThrowException_WhenUserIsNotTheStudent() {
    ...
    assertEquals(
        "Apenas o estudante destinatário pode recusar o contrato.", exception.getMessage());
  }
  ...
  void createContract_ShouldThrowException_WhenChatNotFound() {
    ContractRequestDTO request =
        new ContractRequestDTO(
            50L, LocalDate.now(), LocalDate.now().plusMonths(6), BigDecimal.valueOf(500.0));
  ...
  void createContract_ShouldThrowException_WhenUserIsNotOwner() {
    ...
    assertEquals(
        "Apenas o proprietário do imóvel pode enviar uma proposta de contrato.",
  ...
  void createContract_ShouldCreateContract_WhenAllValidationsPass() {
    ContractRequestDTO request =
        new ContractRequestDTO(
            50L, LocalDate.now(), LocalDate.now().plusMonths(6), BigDecimal.valueOf(600.0));
    when(chatRepository.findById(50L)).thenReturn(Optional.of(chat));
  ...
  void getContractsByChat_ShouldReturnContracts_WhenUserIsStudent() {
    ...
    java.util.List<ContractResponseDTO> contracts =
        contractService.getContractsByChat(50L, student);
