...
public class UserController {
  ...
  public ResponseEntity<UserResponseDTO> updateProfile(
      @AuthenticationPrincipal User currentUser, @RequestBody UpdateUserDTO dto) {
    ...
    if (userUpdated.getTelefones() != null) {
      response.setPhones(userUpdated.getTelefones().stream().map(Telefone::getNumero).toList());
    }
