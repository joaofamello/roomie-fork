...
public class NotificationService {
  ...
  public void notifyOwnerAboutInterest(User owner, User student, Property property) {
    String message =
        student.getName() + " demonstrou interesse no seu imóvel \"" + property.getTitle() + "\".";
    Notification notification = new Notification(owner, message);
    notificationRepository.save(notification);
    log.info(
        "Notificação criada para {} sobre interesse em '{}'",
        owner.getEmail(),
        property.getTitle());
  }
  ...
  public void markAllAsRead(User user) {
    List<Notification> unread =
        notificationRepository.findByRecipientOrderByCreatedAtDesc(user).stream()
            .filter(n -> !n.isRead())
            .toList();
