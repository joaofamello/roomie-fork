...
public class ChatService {
  ...
  /**
   * Inicia um chat entre um proprietário e um estudante para um imóvel. Se já existir um chat entre
   * as partes, retorna o existente.
   */
  @Transactional
  public ChatResponseDTO startChat(ChatRequestDTO request, User owner) {
    ...
    return chatRepository
        .findByStudentIdAndUserIdAndPropertyId(student.getId(), owner.getId(), property.getId())
  ...
  /**
   * Retorna todos os chats do usuário logado (proprietário ou estudante). Busca por ID puro em
   * ambos os papéis.
   */
  ...
  public List<ChatResponseDTO> getChatsForUser(User user) {
    ...
    for (Chat c : asOwner) {
      if (seen.add(c.getId())) allChats.add(c);
    }
    for (Chat c : asStudent) {
      if (seen.add(c.getId())) allChats.add(c);
    }

    return allChats.stream().map(chat -> toDTO(chat, user)).collect(Collectors.toList());
  }
  ...
  private ChatResponseDTO toDTO(Chat chat, User currentUser) {
    long unreadCount =
        messageRepository.countByChatIdAndReadFalseAndSenderIdNot(
