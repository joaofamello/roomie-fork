...
public class ExpenseService {
  ...
  public ExpenseSummaryDTO getExpensesByProperty(Long propertyId) {
    ...
    List<ExpenseResponseDTO> dtos =
        expenses.stream().map(ExpenseResponseDTO::new).collect(Collectors.toList());

    BigDecimal totalAmount =
        expenses.stream().map(Expense::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);

    List<Contract> activeContracts =
        contractRepository.findByPropertyIdAndStatus(propertyId, ContractStatus.ACTIVE);
    ...
    if (numberOfResidents > 0 && totalAmount.compareTo(BigDecimal.ZERO) > 0) {
      amountPerResident =
          totalAmount.divide(BigDecimal.valueOf(numberOfResidents), 2, RoundingMode.HALF_UP);
    }
